...
def dataframe(tickers, start, end):
    df_list = []
    separate_df_dict = {}
    ...
    for ticker in tickers:
        ...
        current_ticker['10 Day Moving Average'] = current_ticker.iloc[:,6].rolling(window=10).mean()
        current_ticker['10 DAY MAC'] = current_ticker.iloc[:,8].diff()

        # EMA's, MACD, Bollinger Bands
        for i in range(len(current_ticker)):
            current_ticker['12 Day EMA'] = current_ticker.iloc[:,6].ewm(span=12).mean()
            current_ticker['26 Day EMA'] = current_ticker.iloc[:,6].ewm(span=26).mean()
            current_ticker['MACD'] = current_ticker['12 Day EMA'] - current_ticker['26 Day EMA']

            current_ticker['30 Day MA'] = current_ticker['Adj Close'].rolling(window=20).mean()

            # set .std(ddof=0) for population std instead of sample
            current_ticker['30 Day STD'] = current_ticker['Adj Close'].rolling(window=20).std() 
    
            current_ticker['Upper Band'] = current_ticker['30 Day MA'] + (current_ticker['30 Day STD'] * 2)
            current_ticker['Lower Band'] = current_ticker['30 Day MA'] - (current_ticker['30 Day STD'] * 2)
...
stock_data = dataframe(tickers, stocks_start, stocks_end)
print(stock_data)
...
print('Min Date = ' + str(stock_data.index.min()))
print('Max Date = ' + str(stock_data.index.max()))
