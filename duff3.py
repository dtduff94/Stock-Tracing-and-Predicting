...
stocks_start = datetime.datetime(2009, 3, 9)
stocks_end = datetime.datetime(2021, 3, 11)
...
for ticker in tickers:
    ...
    # RSI
    #close = current_ticker['Close']
    #current_ticker['RSI'] = talib.RSI(close, timeperiod = 14) #column 15
    
    # EMA's, MACD, Bollinger Bands
    for i in range(len(current_ticker)):
        current_ticker['12 Day EMA'] = current_ticker['Close'].ewm(span=12).mean() #column 16
        current_ticker['26 Day EMA'] = current_ticker['Close'].ewm(span=26).mean() #column 17
        current_ticker['MACD'] = current_ticker['12 Day EMA'] - current_ticker['26 Day EMA'] #column 18

        current_ticker['30 Day MA'] = current_ticker['Adj Close'].rolling(window=20).mean() #column 19

        # set .std(ddof=0) for population std instead of sample
        current_ticker['30 Day STD'] = current_ticker['Adj Close'].rolling(window=20).std() #column 20
    
        current_ticker['Upper Band'] = current_ticker['30 Day MA'] + (current_ticker['30 Day STD'] * 2) #column 21
        current_ticker['Lower Band'] = current_ticker['30 Day MA'] - (current_ticker['30 Day STD'] * 2) #column 22
        current_ticker['Band Percentage'] = current_ticker['Close'] / current_ticker['30 Day MA'] #column 23
        current_ticker['Lower Band Percentage'] = current_ticker['Close'] / current_ticker['Lower Band'] #column 24
        current_ticker['Upper Band Percentage'] = current_ticker['Close'] / current_ticker['Upper Band'] #column 25
        current_ticker['Close to Lower Diff'] = current_ticker['Close'] - current_ticker['Lower Band'] #column 26
        current_ticker['Mid to Lower Diff'] = current_ticker['30 Day MA'] - current_ticker['Lower Band'] #column 27
        current_ticker['Upper to Lower Diff'] = current_ticker['Upper Band'] - current_ticker['Lower Band'] #column 28
        current_ticker['Lower Base'] = 0.00000000000000001 #column 29

    current_ticker['9 Day MACD EMA'] = current_ticker['MACD'].ewm(span=9).mean() #column 31
    for i in range(len(current_ticker)):
        current_ticker['MACD Convergence/Divergence'] = current_ticker['MACD'] - current_ticker['9 Day MACD EMA'] #column 32

    current_ticker['Divergence Change'] = current_ticker['MACD Convergence/Divergence'].diff() #column 33

    close = current_ticker['Close']
    current_ticker['RSI'] = talib.RSI(close, timeperiod = 14) #column 15
    current_ticker['RSI Change'] = current_ticker['RSI'].diff()

    n = 20
    current_ticker['min'] = current_ticker.iloc[argrelextrema(current_ticker["Adj Close"].values, np.less_equal, order=n)[0]]['Adj Close']
    current_ticker['max'] = current_ticker.iloc[argrelextrema(current_ticker["Adj Close"].values, np.greater_equal, order=n)[0]]['Adj Close']

    current_ticker.ffill()
    ...
    try:
        ...
        current_ticker['Market Cap'] = (current_ticker['Adj Close'] * num_of_shares) #column 34
    
        beta_five_year_monthly = rows[3].findAll("span")[0].text
        current_ticker['Beta Five Year Monthly'] = beta_five_year_monthly #column 35
        
        pe_ratio_ttm = rows[5].findAll("span")[0].text
        current_ticker['PE Ratio (TTM)'] = pe_ratio_ttm #column 36
        
        eps_ttm = rows[7].findAll("span")[0].text
        current_ticker['EPS (TTM)'] = eps_ttm #column 37
        
        earnings_date = rows[9].findAll("span")[0].text
        current_ticker['Earnings Date'] = earnings_date #column 38
        
        one_year_target_est = rows[15].findAll("span")[0].text
        current_ticker['One Year Target Estimate'] = one_year_target_est #column 39
...
# Plot Min/Max Data
plt.scatter((separate_df_dict['DDOG'])['Date'], (separate_df_dict['DDOG'])['min'], c='r')
plt.scatter((separate_df_dict['DDOG'])['Date'], (separate_df_dict['DDOG'])['max'], c='g')
plt.plot((separate_df_dict['DDOG'])['Date'], (separate_df_dict['DDOG'])['Adj Close'])
plt.show()
...
# Stock Dates
stock_dates = all_tickers.index.unique()

print(stock_dates)

def trader(portfolio, dataframe):
    for date in stock_dates:
        #if dataframe.loc[(dataframe.index == date) & (dataframe['MACD Convergence/Divergence'] < 0)
        #pd.set_option('display.max_rows', None)
        #print((dataframe.loc[(dataframe.index == date)]).nsmallest(220, 'RSI'))
        #print((dataframe.loc[(dataframe.index == date) & (dataframe['RSI'] > dataframe['RSI'][-2])]).nsmallest(220, 'RSI'))
        #print((dataframe.loc[(dataframe.index == (date - timedelta(days = 1)))]).nlargest(10, 'RSI'))
        #if dataframe.loc[(dataframe.index == date) & (dataframe['RSI'] > 70) & (dataframe.
        
        '''

        print(((dataframe.loc[(dataframe.index == date) & (dataframe['Lower Band Percentage'] > 1)]).nsmallest(10, 'Lower Band Percentage')).nsmallest(5, 'Upper Band Percentage'))
        for i in range(len(current_ticker)):
...
print('Min Date = ' + str(all_tickers.index.min()))
print('Max Date = ' + str(all_tickers.index.max()))
